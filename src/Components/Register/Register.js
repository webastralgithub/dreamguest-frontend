...
function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const { firstName, lastName, email, password, confirm_password, mobile } = formData;

      if (!firstName || !lastName || !email || !password || !confirm_password || !mobile) {
      ...
      if (mobile.length !== 10 || !/^\d+$/.test(mobile)) {
        handleShowModal('Invalid Mobile Number');
        setisSpinnerActive(false);
        return;
      }

      const response = await axios.post(process.env.REACT_APP_API_REGISTER, formData, {
        headers: {
              ...
              <div className='sign-in-form-wrp'>
                <Form onSubmit={handleRegister}>
                  ...
                  <Form.Group className="mb-3" controlId="formGridMobile">
                    ...
                    <Form.Control type="text" maxLength="10" pattern="[0-9]*" name="mobile" placeholder="Enter your Mobile Number" value={formData.mobile} onChange={handleChange} />
                  </Form.Group>
